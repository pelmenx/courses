...
def name_lists(FullNames):
    ItemList=[]
    FirstNameList=[]
    for items in FullNames:
        ItemList.append(items.split(" "))
    for items in ItemList:
        FirstNameList.append(items[0])
    UniqueFirstNameList = []
    for items in FirstNameList:
        if items not in UniqueFirstNameList:
            UniqueFirstNameList.append(items)
    dictionary={}
    FullNames.sort()
    for items in UniqueFirstNameList:
        dictionary[items]=[]
        for names in FullNames:
            if items in names:
                dictionary[items].append(names)
    return dictionary
