...
def abstract_names(name_list):
    dictionary = {}
    for groups in name_list:
        name = ' '.join(map(str, groups))
        name = name.split(" ")
        surname_list = []
        for names in groups:
            surname_list.append((names.split(" ")[1]))
        surname_list.sort()
        dictionary[name[0]] = surname_list
    return dictionary
...
names = [["David Joyner", "David Tennant", "David Beckham"], ["Ananya Birla", "Ananya Agarwal",
                                                              "Ananya Chatterjee", "Ananya Roy"], ["Ines Sainz", "Ines Suarez", "Ines Melchor"]]
