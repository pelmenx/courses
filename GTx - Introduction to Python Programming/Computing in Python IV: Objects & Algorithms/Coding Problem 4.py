...
def check_horse_winner(horse_list):
    count = 0
    winner_string = ""
    for i in range(0, len(horse_list)):
        if horse_list[i] != "HORSE":
            count += 1
            if count == 1:
                winner_string += str(i)
            else:
                winner_string += ", " + str(i)
    if count == len(horse_list):
        return "Everyone: keep playing!"
    elif count == 1:
        return "Player " + winner_string + " wins!"
    else:
        return "Players " + winner_string + ": keep playing!"
