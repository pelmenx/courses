def share_time(calendar, break_time):
    free_time = []
    if break_time[0] < calendar[0][0]:
        free_time.append([])
        free_time[0].append(break_time[0])
        ...
        for i in range(0, len(calendar) - 1):
            if calendar[i][1] != calendar[i + 1][0]:
                ...
                j += 1
    if break_time[1] > calendar[len(calendar) - 1][1]:
        free_time.append([])
        free_time[len(free_time) - 1].append(calendar[len(calendar) - 1][1])
        free_time[len(free_time) - 1].append(break_time[1])
    return free_time
    ...
    for item1 in shift_1:
        for item2 in shift_2:
            if (item2[0] <= item1[1]) and (item1[0] <= item2[1]):
                ...
                j += 1
    return cross_time_more_than(AND_time, minutes)


def cross_time_more_than(array, minutes):
    result = []
    for item in array:
...
calendar_1_break = ["08.00", '20.00']

calendar_2 = [["10.00", '11.30'], ["12.30", '14.30'],
              ["14.30", '15.00'], ["16.00", '17.00']]
calendar_2_break = ["08.00", '18.30']
meeting_duration = 30


avaliable_time_1 = share_time(calendar_1, calendar_1_break)
avaliable_time_2 = share_time(calendar_2, calendar_2_break)
